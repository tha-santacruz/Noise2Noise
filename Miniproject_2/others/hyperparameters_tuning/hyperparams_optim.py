...
def train_model(lr, ndata, bs, augment, project_number = 2):
    ...
    model = Model(lr=lr, bs=bs, augment=augment)
    ...
    for session in tqdm(range(num_train_sessions)):
        ...
        model.save_trained_model(model_name=f"checkpoints/checkpoint_number_{session}.pkl")
    ...
    for lr in hyperparameters["lr"]:
        for augment in hyperparameters["augment"]:
            for batch_size in hyperparameters["batch_size"]:
                print(f"training with lr {lr}, batch size {batch_size}, augment {augment} on {num_data} data examples")
                train_model(lr=lr, ndata= num_data, bs = batch_size, augment=augment)